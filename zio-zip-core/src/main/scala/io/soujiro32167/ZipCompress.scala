...
object ZipCompress {
  ...
  def zip0[R0, R <: R0]: ZPipe[R0, R & Blocking, Throwable, Throwable, ZipEntry[R0, Throwable], Byte] =
    zip(errorMapper = identity)

  def zip[R0, R <: R0, E0, E >: E0](chunkSize: Int = ZStream.DefaultChunkSize, errorMapper: Throwable => E): ZPipe[R0, R & Blocking, E0, E, ZipEntry[R0, E0], Byte] = {
    sources =>
      (for {
  ...
  def unzip[R0, R <: R0](chunkSize: Int = ZStream.DefaultChunkSize): ZPipe[R0, R & Blocking, Throwable, Throwable, Byte, ZipEntry0] = {
    stream =>
